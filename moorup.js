...
class Neuron {
    ...
    async attack(from, rev = false) {
        ...
        setTimeout(async function () {
            ...
            if (from == 'legL') {
                await neurons['legL'].attack(A.part, true)
            }

            if (from == 'legR') {
                await neurons['legR'].attack(A.part, true)
            }

            if (A.part.indexOf('leg') != -1 && rev) {
                resultTime = takingTime
            }
            if (A.part == 'head') {
                resultTime2 = takingTime2
            }
        }, time * 20000);
...
(() => {
    ...
    neurons.there.axonTo2 = neurons['legL']
...
function render() {
    ...
    ctx.fillText(`반응시간(무릎) : ${resultTime.toFixed(3) * 1000}ms`, 600, 150);
    ctx.fillText(`반응시간(대뇌) : ${resultTime2.toFixed(3) * 1000}ms`, 600, 200);
